...
window.config = {
	...
	summerStart: {month: 3, day: 15},  // earliest possible date
	summerEnd: {month: 10, day: 31},  // latest possible date
	targetYear: undefined  // year for which the start/end dates are valid (for UI only at this point)
};
...
$(function(){
	...
	// init history pop-up menu
	for (var key of Object.keys(params.history).sort().reverse()) {
		var enabled = params.history[key] ? '' : ' DISABLED';
		$('#strategy')[0].basedata.innerHTML += '<OPTION VALUE="' + key + '"' + enabled + '>' + key + '</OPTION>';
	}
	...
	function reset () {
		...
		for (var i = 0; i < params.members.length; i++) {
			...
			if (! params.members[i].id) { params.members[i].id = config.nextId++; }
		}
...
//	});
	...
	var targetYearWarning = getSuggestionDates(config).year != config.targetYear ? '<IMG SRC="' + SKGB.StegdienstListeWarning.prototype.iconsPath + 'warning-caution.png" WIDTH=24 HEIGHT=24 ALT="⚠" STYLE="vertical-align:top"> ' : '';
	$('#targetyear')[0].innerHTML = targetYearWarning + config.targetYear;
...
function getSuggestionDates (config) {
	...
	var startDate = new Date(year, config.summerStart.month - 1, config.summerStart.day - 1, 12);
	while (startDate.getDay() !== config.markWeekDay) {
		startDate.setDate(startDate.getDate() + 1);
	}
	
	var endDate = new Date(year, config.summerEnd.month - 1, config.summerEnd.day - 7, 12);
	...
	return { start: startDate, length: weekCount, end: endDate, year: year };
}
...
SKGB.StegdienstListe.prototype.generateShuffledSuggestions = function (members, dates, membersPerDate, smart) {
	...
	for (var i = 0; i < memberAssignees.length; i++) {
		...
		if (smart && i >= membersShuffled.length * 2) {
			...
			while ( membersShuffled[j].avoidMax ) {
...
SKGB.StegdienstListeInterface.prototype.generateSuggestions = function (dates) {
	...
	if (strategy == 'random') {
		...
		this.liste.importData(members, dates, membersPerDate, params.history[historyKey]);
	}
...
//	dataEncoded = encodeURIComponent(dataEncoded.replace(/\//g, '_').replace(/\+/g, '-').replace(/=/g, ''));  // RFC4648
	...
	location.href = config.printAction + "?start=" + firstDate + "&liste=" + dataEncoded;
}
...
SKGB.StegdienstListeWarningList.prototype.iconsPath = "lib/icons/";
...
SKGB.StegdienstListeWarningList.prototype.updateMemberWarningsHtml = function () {
	for (var i = 0; i < this.liste.members.length; i++) {
		...
		// besser indizieren?
		var statisticsHtml = '' + member.count + ' <IMG SRC="' + this.iconsPath + 'warning-ok.png" WIDTH=16 HEIGHT=16 ALT="">';
...
*/
		...
		var statisticsHtml = '<IMG SRC="' + this.iconsPath + 'warning-ok.png" WIDTH=16 HEIGHT=16 ALT="">';
...
SKGB.StegdienstListeWarning.prototype.iconsPath = "lib/icons/";
...
SKGB.StegdienstListeWarning.prototype.asHtml = function () {
	if (this.severity == 'ok') {
		return '<IMG SRC="' + this.iconsPath + 'warning-ok.png" WIDTH=16 HEIGHT=16 ALT="">';
	}
	else if (this.severity == 'note') {
		return '<IMG SRC="' + this.iconsPath + 'warning-note.png" WIDTH=16 HEIGHT=16 ALT="' + this.text + '" TITLE="Hinweis: ' + this.text + '" CLASS="warninglink">';
	}
	else if (this.severity == 'check') {
		return '<IMG SRC="' + this.iconsPath + 'warning-caution.png" WIDTH=16 HEIGHT=16 ALT="' + this.text + '" TITLE="Warnung: ' + this.text + '" CLASS="warninglink">';
	}
	else if (this.severity == 'error') {
		return '<IMG SRC="' + this.iconsPath + 'warning-error.png" WIDTH=16 HEIGHT=16 ALT="' + this.text + '" TITLE="Fehler: ' + this.text + '" CLASS="warninglink">';
	}
